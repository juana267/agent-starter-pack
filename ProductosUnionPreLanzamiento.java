...
public class compras_union {
    ...
    public void productosdelivery() {
        ...
        opcion = union.nextInt();
        union.nextLine();
        if (opcion > 0 && opcion <= productosdel.length) {
            System.out.println("Usted va caomprar"+""+productosdel[opcion - 1]+"su precio es "+"S/."+preciodel[opcion - 1]);
            System.out.println("¿Cuantos comprará? Ingrese la cantidad");
            cantidad = union.nextInt();
            carrito[opcion - 1] += cantidad;
            subtotal_extra[opcion - 1] = carrito[opcion - 1] * preciodel[opcion - 1];
            System.out.println("Ud ha pedido " + carrito[opcion - 1] + " de " + productosdel[opcion - 1]);
            System.out.println("¿Desea elegir otro producto?\n" +
                    "1 SI \n" +
                    "2 NO \n" +
                    "3 Menu Principal");
            opcion = union.nextInt();
            union.nextLine();
            if (opcion == 1) {
                this.productosdelivery();
            } else if (opcion == 2) {
                if (subtotal_extra[opcion - 1] >= 58.00) {
                    System.out.println(" se le sumara S/. 8.00 soles por el delivery");
                    total=subtotal_extra[opcion - 1]+8;
